...
/**
 * PyroStreams Dynamic Choice Field Type
 *
 */
class Field_dynachoice
{	
	public $field_type_slug			= 'dynachoice';
	
	public $db_col_type				= 'varchar';

	public $version					= '1.0.0';

	public $author					= array('name'=>'Tersoal', 'url'=>'http://');
	...
	{		
		$choices = $this->_choices_to_array($params['custom']['stream'], $params['custom']['choice_type'], $field->is_required);
	...
	{
		$choices = $this->_choices_to_array($data['stream'], $data['choice_type'], 'no', false);
	...
	public function validate($value, $mode, $field)
	{
		if (($field->field_data['choice_type'] == 'checkboxes' or $field->field_data['choice_type'] == 'multiselect') and is_array($value))
		{
			...
			if ($max and $min and ($max == $min))
			{
				...
				{
					return str_replace('{val}', $max, lang('streams:dynachoice.must_select_num'));
				}
			}
			else
			{
				...
				if (is_numeric($min))
				{
					...
					{
						return str_replace('{val}', $min, lang('streams:dynachoice.must_at_least'));
					}
				}
				...
				if (is_numeric($max))
				{
					...
					{
						return str_replace('{val}', $max, lang('streams:dynachoice.must_max_num'));
					}
				}
	...
	{
		$options = $this->_choices_to_array($params['stream'], $params['choice_type'], 'no', false);
	...
	/**
	 * Data for choice. Get all streams
	 *
	 * @access	public
	 * @param	[string - value]
	 * @return	string
	...
	{
		//Select all streams
		$streams = $this->CI->db->get($this->CI->streams_m->table);
		...
		foreach ($streams->result() as $stream)
		{
			/*if (substr($stream->stream_name, 0, 5) == 'lang:')
			{
				...
				{
					//$this->CI->lang->load($stream->stream_slug . '/' . $stream->stream_slug);
					$this->CI->load->language($stream->stream . '/' . $stream->stream);
					$choices[$stream->stream_namespace][$stream->id] = lang($stream->stream_name);
				}
			}
			else
			{*/
		...
		return form_dropdown('stream', $choices, $value);
	}
	...
	{
		$choices = array(
			'dropdown' 		=> $this->CI->lang->line('streams:dynachoice.dropdown'),
			'multiselect' 	=> $this->CI->lang->line('streams:dynachoice.multiselect'),
			'radio' 		=> $this->CI->lang->line('streams:dynachoice.radio_buttons'),
			'checkboxes'	=> $this->CI->lang->line('streams:dynachoice.checkboxes')
		);
	...
	public function param_min_choices($value = null)
	{
		return array(
				...
				'instructions'	=> $this->CI->lang->line('streams:dynachoice.checkboxes_only')
			);
	}
	 ...
	 * @return	array
	 */
	public function _choices_to_array($stream, $type = 'dropdown', $is_required = 'no', $optgroups = true)
	{
		...
		//Get stream info
		if ( $stream_info = $this->CI->streams_m->get_stream($stream) )
		{
			$stream_title = $stream_info->title_column;
			
			//Get stream entries
			$stream_entries = $this->CI->db->get($stream_info->stream_prefix . $stream_info->stream_slug);
			
			foreach ($stream_entries->result() as $entry)
			{
				$choices[$entry->id] = $entry->$stream_title;
			}
		}
